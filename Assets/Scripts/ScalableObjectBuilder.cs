...
public class ScalableObjectBuilder : MonoBehaviour
{
    ...
    [SerializeField] [Range(0.1f, 1f)] private float scaleAmplitude;
    [SerializeField] [Range(0f, 1f)] private float scaleRatio;

    public void SetScalableObject(ScalableObjectComponent so)
    {
        float size = Random.Range(minSize, maxSize);
        float scaleOverTime = Random.Range(0, scaleRatio);

        so.transform.localScale = new Vector3(size, 0.5f, size);
        so.size = size;
